import { Menu, Search, Settings, HelpCircle, Grip, SlidersHorizontal, CircleDot } from "lucide-react";
    ...
    <header className="flex items-center justify-between px-4 py-2 bg-background text-foreground">
      ...
      <div className="flex items-center gap-4 w-[240px]">
        ...
        <div className="flex items-center gap-2 text-2xl font-medium tracking-tight">
          ...
          <span className="text-[22px] text-[#e8eaed]">PayMail</span>
        </div>
      </div>

      {/* Middle section: Search bar */}
      <div className="flex-1 max-w-[720px] px-2">
        <div className="relative group flex items-center bg-[#3c4043] rounded-full overflow-hidden focus-within:bg-white focus-within:shadow-md transition-all h-[48px]">
          <button className="p-3 ml-1 text-muted-foreground group-focus-within:text-gray-600 hover:bg-white/10 group-focus-within:hover:bg-gray-100 rounded-full transition-colors">
            <Search className="w-[20px] h-[20px]" />
          </button>
          
          <Input 
            className="w-full bg-transparent border-none h-full px-0 text-base text-[#e8eaed] group-focus-within:text-gray-900 placeholder:text-muted-foreground group-focus-within:placeholder:text-gray-500 focus-visible:ring-0 rounded-none shadow-none"
            placeholder="Search mail"
          />
          
          <button className="p-3 mr-1 text-muted-foreground group-focus-within:text-gray-600 hover:bg-white/10 group-focus-within:hover:bg-gray-100 rounded-full transition-colors">
      ...
      <div className="flex items-center gap-1 justify-end w-[240px]">
        <div className="flex items-center gap-1 bg-[#3c4043] rounded-full px-2 py-1 mr-4 cursor-pointer hover:bg-[#4a4d51] transition-colors">
